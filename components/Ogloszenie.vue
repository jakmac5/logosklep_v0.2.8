...
<div v-html="ogloszenia.opis_html">
  fgdfgdghfdfg
  </div>
<FileUpload :bucket_path="$route.params.id" dbcol="test_upload_konkurs" :dbdoc="$route.params.id"/>
<p>Liczba aktywnych konkursów:  {{urls_to_go.length}}</p>

<div class="columns" v-for="(logos,i) in chunkedLogos" :key='logos.index'>
  <div @mouseover="hover = [i,j]"
        @mouseleave="hover = false" class="column" v-for="(logo,j) in logos" :key="logo.index" >
  ...
  // Now we get the references of these images
        ...
        methods:{
          getKonkursInfo(konkursId){
             ...
             db.collection('test_upload_konkurs').doc(konkursId).onSnapshot(snapshot => {
       if (snapshot.exists) {
      ...
      this.urls_to_go = []  
          let storageRef = Firebase.storage().ref(konkursId)
